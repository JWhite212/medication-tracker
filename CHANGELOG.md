...
- **The landing page has a product walkthrough.** A 43.5-second, 16:9 loop under the hero shows ten features on the demo account: My Day, Quick Log, the medications list, live timers, adding a medication, dose history, analytics, two-factor security, export, and an overdue push reminder on a phone. It is drawn live from Svelte components rather than shipped as a video file, so it is a few KB, stays sharp at any size and follows the app's own tokens and accent maths. Every frame is a pure function of one time value (`walkthrough/timeline.ts`, unit-tested), and the player pauses when scrolled away or the tab is hidden, never autoplays under `prefers-reduced-motion`, always offers a pause button, and gives screen readers a text list of the features instead of the animation. Only the title card is prerendered; the rest mounts when the section nears the viewport.
