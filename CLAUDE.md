...
- Notification `tag`s are a replace-key: showing two notifications with the same tag leaves only the last one. The test notification therefore uses `TEST_PUSH_TAG` from `src/lib/utils/push.ts`, never the reminder tag — sharing it would let "send test" silently clear a real reminder out of the tray. That constant lives in `utils/` (not `server/`) because the service worker and the settings page both need it, and neither may import `$lib/server`.
