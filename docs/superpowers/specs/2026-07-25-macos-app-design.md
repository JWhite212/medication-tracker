...
| `dateFormat` preference **was** write-only dead code on the web (stored, never read) — since fixed there                              | **Implement** it on the Mac (wire it into `DateFormatter`) — it should do what it says. No longer a divergence: the web now reads it through `formatUserDate` (`utils/time.ts`). Match its split — the preference picks the field **order**, each surface picks which fields it shows — or the Mac will disagree with the web on the same date. Two of the three orders map to a locale (`en_GB`, `en_US`); **`YYYY-MM-DD` must not**. Build it from `DateComponents` (the `formatToParts` equivalent), because a locale's field order and separator are CLDR data that may change — `en_CA` already changed once in ICU 72 — so `DateFormatter(locale: en_CA)` is not guaranteed to emit ISO. See `isoDate` in `utils/time.ts`. |
