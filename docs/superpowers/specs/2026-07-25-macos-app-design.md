...
| `dateFormat` preference **was** write-only dead code on the web (stored, never read) — since fixed there                              | **Implement** it on the Mac (wire it into `DateFormatter`) — it should do what it says. No longer a divergence: the web now reads it through `formatUserDate` (`utils/time.ts`). Match its split — the preference picks the field **order** (locale `en_GB`/`en_US`/`en_CA`), each surface picks which fields it shows — or the Mac will disagree with the web on the same date. |
