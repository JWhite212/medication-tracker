...
<div class="group relative rounded-xl border border-glass-border bg-glass backdrop-blur-xl transition-colors hover:bg-glass-hover">
  <a href="/medications/{medication.id}" class="block p-4">
    <div class="flex items-center gap-4">
      <div class="h-10 w-10 shrink-0 rounded-lg" style="background: {getMedicationBackground(medication.colour, medication.colourSecondary, medication.pattern)}"></div>
    ...
    {#if isScheduled}
      <div class="mt-2 flex items-center gap-2">
        <div class="h-1.5 flex-1 overflow-hidden rounded-full bg-glass">
          <div
            class="h-full rounded-full transition-all"
            style="width: {adherencePercent}%; background: {medication.colour}"
          ></div>
        </div>
        <span class="shrink-0 text-xs tabular-nums text-text-muted">{adherencePercent}%</span>
      </div>
    {/if}
  </a>
  ...
  <button
    type="button"
    class="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-lg bg-glass text-text-secondary opacity-0 transition-all hover:bg-accent hover:text-accent-fg group-hover:opacity-100"
