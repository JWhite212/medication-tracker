...
{#if groups.length > 0}
  <section>
    <h2 class="mb-3 text-sm font-medium uppercase tracking-wider text-text-muted">My Day</h2>
    <div class="space-y-3">
      {#each groups as group (group.key)}
        <div class="rounded-xl border border-glass-border bg-glass p-4 backdrop-blur-xl">
          <h3 class="mb-2 flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-text-secondary">
          ...
          <div class="space-y-1.5">
            {#each group.slots as slot (`${slot.medicationId}-${slot.expectedTime}`)}
              ...
              <div class="flex items-center gap-3 rounded-lg px-2 py-1.5 transition-colors hover:bg-glass-hover">
                <!-- Status indicator -->
                {#if slot.status === 'taken'}
                  <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-success/20 text-success" aria-label="Taken">
                ...
                {:else if slot.status === 'overdue'}
                  <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-warning/20 text-warning" aria-label="Overdue">
                    <svg class="h-3 w-3" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                {:else}
                  <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-glass-border" aria-label="Upcoming">
                    <span class="h-2 w-2 rounded-full bg-text-muted/40"></span>
                  </span>
                {/if}
                ...
                <span
                  ...
                  style="background: {getMedicationBackground(slot.colour, slot.colourSecondary, slot.pattern, true)}"
                ></span>
                ...
                {#if slot.status !== 'taken'}
                  ...
                  >
                    ...
                    <button
                      type="submit"
                      class="rounded-md px-2 py-0.5 text-xs font-medium text-accent transition-colors hover:bg-accent/10"
