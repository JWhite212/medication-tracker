  ...
  <div class="flex flex-col gap-3">
    {#each MEDICATIONS as medication, i (medication.id)}
      ...
      <div class="flex items-center gap-2">
        <!-- Reorder arrows; the first can't move up and the last can't move down -->
        <div class="flex flex-col gap-0.5">
          <span
            class="text-text-muted rounded-xs px-1.5 py-0.5 text-xs {i === 0 ? 'opacity-30' : ''}"
            >▲</span
          >
          <span
            class="text-text-muted rounded-xs px-1.5 py-0.5 text-xs {i === MEDICATIONS.length - 1
              ? 'opacity-30'
              : ''}">▼</span
          >
        </div>
        <div use:mark={"m.card" + i} class="flex-1">
          <div class="border-glass-border bg-glass relative rounded-xl border">
            <div class="p-4">
