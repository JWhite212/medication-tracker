...
export async function getDailyDoseCounts(userId: string, days: number) {
  const since = new Date(Date.now() - days * 86400000);
  return db
    .select({
      date: sql<string>`date(${doseLogs.takenAt} AT TIME ZONE 'UTC')`,
      count: sql<number>`count(*)::int`,
    })
    .from(doseLogs)
    .where(and(eq(doseLogs.userId, userId), gte(doseLogs.takenAt, since)))
    .groupBy(sql`date(${doseLogs.takenAt} AT TIME ZONE 'UTC')`)
    .orderBy(desc(sql`date(${doseLogs.takenAt} AT TIME ZONE 'UTC')`));
}
...
export async function getHourlyDistribution(userId: string, days: number) {
  const since = new Date(Date.now() - days * 86400000);
  return db
    .select({
      hour: sql<number>`extract(hour from ${doseLogs.takenAt})::int`,
      count: sql<number>`count(*)::int`,
    })
    .from(doseLogs)
    .where(and(eq(doseLogs.userId, userId), gte(doseLogs.takenAt, since)))
    .groupBy(sql`extract(hour from ${doseLogs.takenAt})`)
    .orderBy(sql`extract(hour from ${doseLogs.takenAt})`);
}
