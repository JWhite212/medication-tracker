...
// hydration (`bind:group`; `mount()` imported into a test file here resolves
// to Svelte's server entry and throws, so the suite has no client-mount
// harness), or how the check mark and the focus ring look (CSS). The class
// assertions below pin which STATE drives each cue, not the pixels.
//
...
describe("MedicationStylePicker: a stored colour outside the presets", () => {
  ...
  it("does not mistake the empty secondary a failed submit echoes back for a colour", () => {
    ...
    // the echoed value back in on a validation failure.
    const doc = renderStylePicker({ colour: "#6366f1", secondary: "" });
    expect(radios(doc, "colourSecondary")).toHaveLength(PRESET_COLOURS.length);
    expect(radios(doc, "colourSecondary").map(nameOf)).not.toContain(CUSTOM_COLOUR_NAME);
  });
});
